...
import InterestCard from './InterestsCard/InterestCard';
import NameModal from '../../../../containers/NameModalContainer/NameModalContainer';
/*
...
class MainInterests extends React.Component {
    ...
    render() {
        const interestsBoxes = this.interests.map((json, index) => {
            ...
            const nextCard = (!this.interests[nextIndex]) ? <div className={classNames(interestsCss.hold)}/> : <InterestCard key={nextIndex} stateInterests={this.stateInterests} json={nextJson} index={nextIndex} onClickInterest={this.onClickInterest} />;
            if(index % 2 === 0) {
                return (
